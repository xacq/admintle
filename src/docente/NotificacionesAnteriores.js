...
const NotificacionesAnteriores = () => {
  ...
  const { data: notificaciones, loading, error } = useApiData('/api/notificaciones?category=anterior');
  ...
  const filtered = useMemo(() => {
    ...
    return notificaciones.filter((notificacion) =>
      [notificacion.titulo, notificacion.descripcion, notificacion.hasta]
        .filter(Boolean)
        .some((value) => value.toLowerCase().includes(term))
    );
  }, [notificaciones, searchTerm]);

  const visible = filtered.slice(0, recordsPerPage);
                    ...
                    <tr key={notificacion.id}>
                      ...
                      <td>
                        <div className="action-buttons">
                          <button className="action-btn view-btn" title="Ver" type="button">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                              <circle cx="12" cy="12" r="3"></circle>
                            </svg>
                          </button>
                          <button className="action-btn edit-btn" title="Editar" type="button">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                            </svg>
                          </button>
                          <button className="action-btn delete-btn" title="Eliminar" type="button">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
