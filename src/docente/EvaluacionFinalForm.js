...
const EvaluacionFinalForm = () => {
  ...
  const resumen = useMemo(() => {
    ...
    return {
      totalEvaluados,
      promedioGeneral,
      ultimaFecha: ultimaFechaRegistro ? ultimaFechaRegistro.toLocaleDateString('es-BO') : '—',
  ...
  const handleEvaluar = (beca) => {
    ...
    setFormData({
      ...
      calificacionFinal:
        beca.evaluacionFinal?.calificacionFinal !== null &&
        beca.evaluacionFinal?.calificacionFinal !== undefined
  ...
  const handleGuardarEvaluacion = async () => {
    ...
    try {
      ...
      if (data?.id) {
        setBecas((prev) => prev.map((beca) => (beca.id === data.id ? { ...beca, ...data } : beca)));
      }
